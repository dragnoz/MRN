...
exports.parseFileForIdentifiers = parseFileForIdentifiers;
...
function getJsonKeysToScan() {
    // Consider adding a default list if config is empty
    return vscode.workspace.getConfiguration('mrt').get('linker.jsonKeysToScan', [
        "identifier",
        "textures",
        "geometry",
        "materials",
...
function parseFileForIdentifiers(document) {
    const result = { identifiers: new Set(), calledFunctions: new Set() };
...
// --- Parsing Functions --- 
function parseJsonContent(content, identifiers) {
    try {
...
function recursivelyFindIdentifiersInJson(data, identifiers, currentKey) {
...
function parseMcfunctionContent(content, result) {
...
function parseLangContent(content, identifiers) {
...
// --- Helper Function --- 
function findMatches(content, regex, collection, captureGroup = 0) {
    let match;
